...
function fun2c2p_init() {
	...
	class WC_Gateway_2c2p extends WC_Payment_Gateway {
		...
		public function __construct(){
			...
			$test_title			= '';	
			$test_description	= '';
			if ( $this->settings['test_mode'] == 'demo2' ) {
				$demo = '2C2PFrontEnd/';
			...
			$this->service_provider	= $this->settings['service_provider']; // The Service options for 2c2p.
			$this->version_api      = $this->settings['version_api'];//VS1006 - version for api
			$this->currency         = $this->settings['currency_select'];//VS1006 - currency for api
			$this->redirect_page1	= $this->settings['result_url_1'];//VS1006 - to redirect user after transaction
			$this->redirect_page2	= $this->settings['result_url_1'];//VS1006 - to redirect from backend
            $this->msg['message']	= '';
            $this->msg['class'] 	= '';
		...
		function init_form_fields(){
			...
			$this->form_fields = array(
				...
				'service_provider' => array(
					'title' 		=> __('Service Provider:', 'woo_2c2p'),
					'type' 			=> 'select',
					'options' 		=> array('money'=>'2C2P')
				),
				// LIVE Key-ID
      			'key_id' => array(
    			...
    			'key_secret' => array(
      			...
      			'test_mode' => array(
					...
					'options' 		=> array('demo2'=>'Test Mode','t'=>'Live Mode'),
					'default' 		=> 'test',
					'description' 	=> __('Mode of 2c2p activities'),
					'desc_tip' 		=> true
                ),
				//------------VS1006---------------//
				// Version for payment gateway api
				'version_api' => array(
					'title'			=> __('Version:', 'woo_2c2p'),
					'type'			=> 'text',
					'description'	=> __('current version of API is 6.4'),
					'desc_tip' 		=> true
				)
		...
		function receipt_page($order){
			echo '<p><strong>' . __('ขอบคุณสำหรับการสั่งซื้อคอร์สออนไลน์', 'woo_2c2p').'</strong><br/>' . __('จะเริ่มการชำระเงินเมื่อคุณคลิกปุ่ม "ชำระเงินด้วย 2C2P".', 'woo_2c2p').'</p>';
		...
		function generate_2c2p_form($order_id){
			...
			// Redirect URL
			/* if ( $this->redirect_page1 == '' || $this->redirect_page1 == NULL ) {
							
			 $redirect_url = get_site_url() . "/";
			
			} else {
			 $redirect_url = $this->redirect_page1;//get_permalink( $this->redirect_page1 );
			} */
			// Redirect URL : For WooCoomerce 2.0
			if ( version_compare(WOOCOMMERCE_VERSION, '2.0.0', '>=' ) ) {
				$redirect_url = add_query_arg( 'wc-api', get_class( $this ), $redirect_url );
				//$redirect_url = $redirect_url;//get_permalink( $this->redirect_page1 );
			}
			
			//--------------VS1006---------------//
			

			$amount_con = $order->order_total;
			$amount_con = number_format($amount_con,2);
			$amount_con = str_replace(".","",$amount_con);
            $amount_con = str_replace(",","",$amount_con);
			...
			// amount
			session_start();
			$_SESSION['amount_con'] = $amount_con;
			...
			$merchant_id = $this->key_id;
            $secret_key = $this->key_secret;
			$payment_description = $time1;
			...
			";
			$getCurrency = json_decode(base64_decode($currencycodeStr), true);
				

			//GET CODE FROM ARRAYSTRING

			foreach($getCurrency as $k=>$value){
					// SEARCH KEY CODE : "$this->session->data['currency']"####

					if (in_array($currencyType,$value)){						
							$ccode = $getCurrency[$k][3];
					}
			}
		...
		//---------------------------------------------------------
			...
			$currency = empty($getccode) ? "764" : $getccode;
			...
			//--------------VS1006---------------/
			if ( $this->service_provider == 'money' ) {
				$service_provider = '';
			} else {
			...
			return '<form name="p2c2pform" action="'.$this->liveurl.'" method="post"/>
				<input type="hidden" id="version" name="version" value="'.$version.'"/>
				<input type="hidden" id="merchant_id" name="merchant_id" value="'.$this->key_id.'">
				...
				<input type="hidden" id="customer_email" name="customer_email"value="'.$order->billing_email.'"/>
				...
				<input type="hidden" id="result_url_1" name="result_url_1" value="'.$redirect_url.'"/>
				<input type="hidden" id="result_url_2" name="result_url_2" value="'.$redirect_url.'"/>
				<input type="hidden" id="hash_value" name="hash_value" value="'.$HashValue.'"/>
				<input type="submit" id="btnSubmit" class="button-alt" id="submit_2c2p_payment_form" value="'.__('ชำระเงินด้วยบัตรเครดิตร ผ่าน 2C2P', 'woo_2c2p').'" /> <a class="button cancel" href="'.$order->get_cancel_order_url().'">'.__('ยกเลิกการชำระเงิน และกลับไปที่หน้ารายการสั่งซื้อ', 'woo_2c2p').'</a>
		  </form>  
	   ';
				
		
		}
		//END-generate_2c2p_form
        ...
        function check_2c2p_response(){	
			...
			// CONFIRM RESPONSE AMOUNT
				$amount_con1 = $woocommerce->cart->total;
				$amount_con1 = str_replace(".","",$amount_con1);
				$amount_con1_length = strlen($amount_con1);

				for($length=$amount_con1_length;$length < 12;$length++){
					$amount_con1 = '0'.$amount_con1;	
				}
			//------------------------
				$order_id = $_REQUEST['order_id'];
				$order_id_2c2p = explode("_",$order_id);
				$orderarray = explode('_',$order_id);
				$order = new WC_Order( $orderarray['0'] );

			// Confirm Response HASH
				extract($_REQUEST);
				session_start();
				$amount_con1 = $_SESSION['amount_con'];
				
				$payment_status_rededined='000';

				$srr="$version$request_timestamp$merchant_id$order_id$invoice_no$currency$amount_con1$transaction_ref$approval_code$eci$transaction_datetime$payment_channel$payment_status_rededined$channel_response_code$channel_response_desc$masked_pan$stored_card_unique_id$backend_invoice$paid_channel$paid_agent$user_defined_1$user_defined_2$user_defined_3$user_defined_4$user_defined_5$browser_info
				";
				$srr = trim($srr);
				$secret_key = $this->key_secret;

				$confirmedHashValue = strtolower(hash_hmac('sha1',$srr,$secret_key,false));	
				
				
				
				//----------------------------------------------------

				// HASH_TAG for payment statue: 001

				$payment_status_rededined='001';

				$srr="$version$request_timestamp$merchant_id$order_id$invoice_no$currency$amount_con1$transaction_ref$approval_code$eci$transaction_datetime$payment_channel$payment_status_rededined$channel_response_code$channel_response_desc$masked_pan$stored_card_unique_id$backend_invoice$paid_channel$paid_agent$user_defined_1$user_defined_2$user_defined_3$user_defined_4$user_defined_5$browser_info";
				$srr = trim($srr);
				$secret_key = $this->key_secret;
				$pendingpaymentHashValue = strtolower(hash_hmac('sha1',$srr,$secret_key,false));
				//echo "<br/>===>PENDING==>".$pendingpaymentHashValue;
				//die('<br/>hash_pending');
				//---------------------------------

					if(strcasecmp($confirmedHashValue,$hash_value) == '0'){
									...
									$amount_2c2p = $woocommerce->cart->total;
									$transaction_ref_2c2p = $_REQUEST['transaction_ref'];
									$status_2c2p = '000';
									$approval_2c2p = $_REQUEST['approval_code'];

									update_post_meta($order_id_2c2p[0], '2c2p_order_id', $order_id_2c2p[0]);
									update_post_meta($order_id_2c2p[0], '2c2p_transaction_id', $transaction_ref_2c2p);
									update_post_meta($order_id_2c2p[0], '2c2p_amount', $amount_2c2p);
									update_post_meta($order_id_2c2p[0], '2c2p_transaction_code', $status_2c2p);
									update_post_meta($order_id_2c2p[0], '2c2p_approval_code', $approval_2c2p);

									if($order_id != ''){
										try{
											if($order->status == 'processing'){
												$order->add_order_note('order_id: '.$_REQUEST['order_id'].'<br/>transaction_ref: '.$_REQUEST['transaction_ref'].'<br/>payment status: '.$_REQUEST['payment_status'].'<br/>amount: '.$_REQUEST['amount'].'<br/>eci: '.$_REQUEST['eci'].'<br/>transaction_datetime: '.$_REQUEST['transaction_datetime'].'<br/>approval_code: '.$_REQUEST['approval_code'].'<br/>amount: '.$_REQUEST['amount']);
												$order->update_status('Completed');
											}else{
												$order->payment_complete();
												$order->add_order_note('2C2P payment transaction successful.<br/>order_id: '.$_REQUEST['order_id'].'<br/>transaction_ref: '.$_REQUEST['transaction_ref'].'<br/>eci: '.$_REQUEST['eci'].'<br/>transaction_datetime: '.$_REQUEST['transaction_datetime'].'<br/>approval_code: '.$_REQUEST['approval_code'].'<br/>amount: '.$_REQUEST['amount'].'<br/>payment status: '.$_REQUEST['payment_status']);
											...
											if( $order->status !=='completed' ){
												...
												switch ($_REQUEST['payment_status']) {
													case "000":
														$note="Success using credit/debit card (Authorized) or Success when paid with cash channel (Paid)";
														$order->update_status('completed');
													break;
													case "001":
														$note="Pending (Waiting customer to pay)";
													break;
													case "002":
														$note="Rejected (Failed payment)";
													break;
													case "003":
														$note="User cancel (Failed payment)";
													break;
													case "999":
														$note="Error (Failed payment)";
													break;
													default :
														//=> Cancel
														$note="Cancel";
												...
												//add_action('the_content', array(&$this, '2c2p_showMessage'));
							...
							}else if(strcasecmp($pendingpaymentHashValue,$hash_value) == '0'){ //Payment Pending

									//die('hash_for_pending');
									$order_id = $_REQUEST['order_id'];								
									$trans_authorised = true;
									...
									echo '<br/>';
									echo "<a href='".get_site_url()."'>Go To Home</a>";
									exit;
        ...
        /**
         * Get Page list from Wocommerce
         **/
		function fun2c2p_get_pages($title = false, $indent = true) {
...
function fun2c2p_add_action_plugin( $actions, $plugin_file ) {
	...
	if ($plugin == $plugin_file) {
			...
			$settings = array('settings' => '<a href="admin.php?page=wc-settings&tab=checkout&section=WC_Gateway_2c2p">' . __('Settings') . '</a>');
