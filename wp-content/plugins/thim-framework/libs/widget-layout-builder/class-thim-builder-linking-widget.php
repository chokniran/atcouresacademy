...
class Thim_Builder_Linking_Widget {
	...
	public static function handle_request_edit_content_widget() {
		...
		$update         = $widget_builder->transfer_content_to_page();
		if ( ! $update ) {
			return;
		}

		$page_redirect = $widget_builder->get_url_page_edit();
	 ...
	 * @param $page_id
	 */
	public static function update_widget_content( $page_id ) {
		$widget_id = self::get_widget_id_linking_page_id( $page_id );
		...
		Thim_Widget_Layout_Builder::update_content( $widget_id, $post->post_content );
	}
