<div class="userpro userpro-<?php echo $i; ?> userpro-id-<?php echo $user_id; ?> userpro-<?php echo $layout; ?>" <?php userpro_args_to_data( $args ); ?>>
	...
	<div class="userpro-centered <?php if (isset($header_only)) { echo 'userpro-centered-header-only'; } ?>">
		...
		<div class="userpro-profile-img" data-key="profilepicture"><a href="<?php echo get_site_url()?>/profile"><?php echo get_avatar( $user_id, $profile_thumb_size ); ?></a></div>
		<?php } else { ?>
		<div class="userpro-profile-img" ><a href="<?php echo get_site_url()?>/profile" title="<?php _e('View Profile','userpro'); ?>"><?php echo get_avatar( $user_id, $profile_thumb_size ); ?></a></div>
		<?php } ?>
		
		<div class="userpro-profile-img-after">
			<div class="userpro-profile-name">
				<a href="<?php echo get_site_url()?>/profile"><?php echo get_user_meta($user_id, 'billing_first_name', true).' '.get_user_meta($user_id, 'billing_last_name', true); ?></a>
		  ...
		  if(userpro_show_badges( $user_id )!='<span class="userpro-badges"></span>')	{?>		
			...
			<div class="userpro-profile-img-btn">
				...
				<a href="<?php echo get_site_url()?>/profile" class="userpro-button secondary"><?php _e('View Profile','userpro') ?></a>
				<?php } else { ?>
				<a href="<?php echo get_site_url()?>/profile" class="userpro-button secondary"><?php _e('ดูโปรไฟล์','userpro') ?></a>
				<?php } ?>
