			...
			<div class="course-price" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
				...
				<?php else: 
											    $price = learn_press_format_price( $course->get_price(), true );
											     $fullprice = get_post_meta( $course_item->ID, '_lp_sale_price', true );
												 if(!empty($fullprice)){
									             ?>
									                <div class="value sale_price" itemprop="price" content="<?php echo esc_attr( $price ); ?>">
									                   	<?php echo esc_html( $price ); ?>
											        </div>
											        <div class="fullprice"><?php echo get_post_meta($course_item->ID, '_lp_sale_price', true );?><?php echo learn_press_get_currency_symbol(); ?></div>
									                 <?php
									             }else{
									             	?>
